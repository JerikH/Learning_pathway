...
;Este programa cálcula el resultado de e^x en función del número ingresado por el usuario y los términos que este determine,
...
(define (serie x ter cont acum valor);inicio de la definición de la función que calcula el valor de e^x
  (if (< cont ter)
      (begin
        (set! cont (+ cont 1))
        (set! acum (/ (expt x cont) (facto cont 1 1 1)))
...
(displayln "Este programa calcula el valor de e^x con los datos ingresados a continuación")
(displayln "Ingrese el valor de x que desea emplear para el procesamiento(para garantizar un correcto resultado ingrese el número que desea con el punto decimal, ejemplo: 2.0) ")
...
(newline)
(printf "Empleando ~a términos para el cálculo el resultado de e^~a es: ~a" ter x (serie x ter cont acum valor))