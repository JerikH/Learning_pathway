...
(define (serie x ter cont acum valor);inicio de la definición de la función que calcula el valor de cosh(x)
  (if (< cont ter)
      (begin
        (set! cont (+ cont 1))
        (set! acum (/ (expt x (* 2 cont)) (facto (* 2 cont) 1 1 1)))
...
(displayln "Este programa calcula el valor de cosh(x) con los datos ingresados a continuación")
...
(printf "Empleando ~a términos para el cálculo el resultado de cosh(~a) es: ~a" ter x (serie x ter cont acum valor))