...
;Este programa cálcula el resultado de in(x) en función del número ingresado por el usuario y los términos que este determine,
...
(define (serie x ter cont acum valor);inicio de la definición de la función que calcula el valor de in(x)
  (if (< cont ter)
      (begin
        (set! cont (+ cont 1))
        (set! acum (/ (* (expt -1 cont) (expt (- x 1) (+ 2 cont))) (+ 2 cont)))
...
(displayln "Este programa calcula el valor de in(x) con los datos ingresados a continuación")
...
(printf "Empleando ~a términos para el cálculo el resultado de in(~a) es: ~a" ter x (serie x ter cont acum valor))