...
;Este programa cálcula el resultado de sen(x) en función del número ingresado por el usuario y los términos que este determine,
...
(define (serie x ter cont acum valor);inicio de la definición de la función que calcula el valor de sen(x)
  (if (< cont ter)
      (begin
        (set! cont (+ cont 1))
        (set! acum (/ (* (expt -1 cont) (expt x (+ 1 (* 2 cont)))) (facto (+ 1 (* 2 cont)) 1 1 1)))
...
(displayln "Este programa calcula el valor de sen(x) con los datos ingresados a continuación")
...
(printf "Empleando ~a términos para el cálculo el resultado de sen(~a) es: ~a" ter x (serie x ter cont acum valor))